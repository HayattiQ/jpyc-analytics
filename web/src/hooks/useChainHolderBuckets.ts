...
export const useChainHolderBuckets = (chainId: string) => {
  const [data, setData] = useState<HolderBucketsData | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const chain = useMemo(() => config.chains.find((c) => c.id === chainId), [chainId])

  const fetchBuckets = useCallback(async () => {
    if (!chain?.subgraphUrl || !chain?.globalStatId) return
    setLoading(true)
    setError(null)
    try {
      ...
      if (!gs) throw new Error('globalStat が取得できません')
  ...
  return { data, loading, error, reload: fetchBuckets, chain }
}
