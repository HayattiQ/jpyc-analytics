...
6. **チェーンフローインサイト**:  
   - **Transaction Volume (日次)**: 過去 30 日をデフォルトとしたチェーン別スタックエリアチャート。The Graph Subgraph から日次スナップショットを取得し、チェーンごとに色分けして積み上げ合計を表示。ツールチップには日付、総量、各チェーンの値を表示し、凡例からチェーンの表示/非表示をトグル可能。  
   - **Net Inflow / Outflow (日次)**: 運営の `issuerAddress` から外部へ送付された Transfer を Inflow、`redeemAddress` へ送付された Transfer を Outflow として集計し、正味流入量を 0 ライン基準のダイバージングバー（正値は上方向、負値は下方向）で表示。プラス域は淡いグリーン、マイナス域はオレンジ〜レッド系で視覚化し、当日値と 7 日平均をオーバーレイ表示する。閾値を超える急変動はバッジで注記。
...
- **チャート群**: 時系列 (発行量、TVL)、棒グラフ (取引件数)、表形式 (トップアドレス)、日次チェーン別スタックエリア (Transaction Volume)、ダイバージングバー (Net Inflow / Outflow)。
- **細部要件**
...
- Transaction Volume / Net Inflow チャートはホバー時にクロスヘアとチェーン別内訳、合計、前日比を表示し、CSV/PNG エクスポートとチェーン単位の表示切替を提供。
...
## 10. データ更新フロー
1. The Graph 側の Subgraph がチェーン上のイベントを継続的にインデックス化し、JPYC 発行量や TVL、日次トランザクションボリューム、issuer/redeem 宛ての Transfer を常時更新（M1 以降の主データ源）。
